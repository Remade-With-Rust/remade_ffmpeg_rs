...
The **AVI (Audio Video Interleaved)** container for **remade_ffmpeg_rs** — our
own code, not a binding.

- **Demux + mux** — RIFF chunk parsing, `hdrl` header list, `movi` data and the `idx1` index.
- Pure Rust, no C/FFI.
...
fn main() -> Result<(), Error> {
    ...
    let file = std::fs::File::open("input.avi").expect("open input");
    let mut demuxer = formats.open_demuxer("avi", Box::new(file))?;
