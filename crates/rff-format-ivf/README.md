...
The **IVF** container for **remade_ffmpeg_rs** — the native VP8/VP9 test
wrapper, compatible with `vpxenc`, `vpxdec` and FFmpeg. Our own code.

- **Demux + mux** — 32-byte file header plus per-frame size/timestamp headers.
- The interchange format for VP9 conformance and BD-rate work; pairs with [`rff-codec-vp9`](https://crates.io/crates/rff-codec-vp9).
- Pure Rust, no C/FFI.
...
fn main() -> Result<(), Error> {
    ...
    rff_format_ivf::register(&mut formats);

    let file = std::fs::File::open("input.ivf").expect("open input");
    let mut demuxer = formats.open_demuxer("ivf", Box::new(file))?;
