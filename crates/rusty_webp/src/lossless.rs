        ...
        while index < num_values {
            ...
            if code < 256 {
                ...
                if dist == 1 {
                    ...
                    // The spec inserts every decoded pixel. A run repeats one
                    // value, so one insert suffices — but it must happen: the
                    // slot may have been evicted since the value was first
                    // inserted, and a later cache reference would then read a
                    // stale entry.
                    if let Some(color_cache) = huffman_info.color_cache.as_mut() {
                        color_cache.insert(value);
                    }
