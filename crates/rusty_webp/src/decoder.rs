...
/// Raw YUV 4:2:0 planes of a lossy (VP8) still image, as reconstructed by the
/// decoder. Returned by [`WebPDecoder::read_yuv420`].
///
/// Planes are macroblock-padded: rows are `y_stride` / `uv_stride` bytes and
/// there may be padding rows past the visible height — consumers must index
/// `width`×`height` (luma) and `ceil(width/2)`×`ceil(height/2)` (chroma)
/// through the strides, never assume tightly-packed planes.
pub struct Yuv420Frame {
    /// Visible image width in luma samples.
    pub width: u32,
    /// Visible image height in luma samples.
    pub height: u32,
    /// Bytes per luma row (width padded to a macroblock multiple).
    pub y_stride: usize,
    /// Bytes per chroma row (`y_stride / 2`).
    pub uv_stride: usize,
    /// Luma plane.
    pub y: Vec<u8>,
    /// Chroma-blue plane, half resolution in both axes.
    pub u: Vec<u8>,
    /// Chroma-red plane, half resolution in both axes.
    pub v: Vec<u8>,
}
...
impl<R: BufRead + Seek> WebPDecoder<R> {
    ...
    /// For still lossy (VP8) images without alpha, returns the raw YUV 4:2:0
    /// planes exactly as reconstructed — no chroma upsampling, no RGB
    /// conversion. Samples are BT.601 limited-range, which is what the VP8
    /// bitstream carries; consumers that want video (Y'CbCr) output can take
    /// these planes directly instead of round-tripping through RGB.
    ///
    /// Returns `Ok(None)` for lossless (VP8L), animated, or alpha-carrying
    /// images — those decode via [`read_image`](Self::read_image).
    pub fn read_yuv420(&mut self) -> Result<Option<Yuv420Frame>, DecodingError> {
        if self.is_animated() || self.has_alpha() || !self.is_lossy() {
            return Ok(None);
        }
        let range = self
            .chunks
        ...
        let y_stride = usize::from(frame.buffer_width());
        Ok(Some(Yuv420Frame {
            width: self.width,
            height: self.height,
            y_stride,
