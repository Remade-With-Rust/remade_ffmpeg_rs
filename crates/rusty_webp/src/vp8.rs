...
pub struct Vp8Decoder<R> {
    ...
    /// Stage nanoseconds: [entropy (header+residual), predict+idct, loop filter, frame header].
    #[cfg(feature = "profile")]
    prof: [u64; 4],
}
...
impl<R: Read> Vp8Decoder<R> {
    ...
    fn read_coefficients(
        ...
        dcq: i16,
        acq: i16,
    ) -> Result<(bool, bool), DecodingError> {
        ...
        // Fast path: read the whole block in one session — one state
        // snapshot and one commit instead of a state copy in and out per
        // read. The reads are the exact sequence of the committed path below.
        {
            let mut s = decoder.begin_session();
            ...
            for i in first..16usize {
                ...
                let token = decoder.session_read_with_tree(&mut s, tree, tree[skip as usize]);
                ...
                let mut abs_value = i32::from(match token {
                    ...
                    category @ DCT_CAT1..=DCT_CAT6 => {
                        ...
                        for t in probs.iter().copied() {
                            ...
                            let b = decoder.session_read_bool(&mut s, t);
                ...
                if decoder.session_read_flag(&mut s) {
                    abs_value = -abs_value;
                }
            ...
            if decoder.commit_session(s) {
                return Ok((has_coefficients, has_ac));
            }
        }

        // The session ran past the buffer end (file tail): its writes into
        // `block` are garbage — reset and redo through the committed
        // per-read path, whose cold half handles the final bytes.
        block.fill(0);
        ...
        decoder.check(res, (has_coefficients, has_ac))
    }
    ...
    fn read_residual_data(
        ...
        if plane == 1 {
            ...
            let (n, _) = self.read_coefficients(&mut block, p, plane, complexity as usize, dcq, acq)?;
        ...
        for y in 0usize..4 {
            ...
            for x in 0usize..4 {
                ...
                let (n, has_ac) = self.read_coefficients(block, p, plane, complexity as usize, dcq, acq)?;
        ...
        if filter_level > 0 {
            ...
            if mbx > 0 {
                ...
                if self.frame.filter_type {
                    ...
                    loop_filter::macroblock_filter_horizontal_batch::<16>(
                        hev_threshold,
                        interior_limit,
                        mbedge_limit,
                        &mut self.frame.ybuf,
                        (mby * 16) * luma_w + mbx * 16 - 4,
                        luma_w,
                    );
                    loop_filter::macroblock_filter_horizontal_batch::<8>(
                        hev_threshold,
                        interior_limit,
                        mbedge_limit,
                        &mut self.frame.ubuf,
                        (mby * 8) * chroma_w + mbx * 8 - 4,
                        chroma_w,
                    );
                    loop_filter::macroblock_filter_horizontal_batch::<8>(
                        hev_threshold,
            ...
            if do_subblock_filtering {
                if self.frame.filter_type {
                    ...
                    for x in (4usize..16 - 3).step_by(4) {
                        loop_filter::subblock_filter_horizontal_batch::<16>(
                            hev_threshold,
                            interior_limit,
                            sub_bedge_limit,
                            &mut self.frame.ybuf,
                            (mby * 16) * luma_w + mbx * 16 + x - 4,
                            luma_w,
                        );
                    }

                    loop_filter::subblock_filter_horizontal_batch::<8>(
                        hev_threshold,
                        interior_limit,
                        sub_bedge_limit,
                        &mut self.frame.ubuf,
                        (mby * 8) * chroma_w + mbx * 8 + 4 - 4,
                        chroma_w,
                    );
                    loop_filter::subblock_filter_horizontal_batch::<8>(
                        hev_threshold,
            ...
            if mby > 0 {
                if self.frame.filter_type {
                    ...
                    loop_filter::macroblock_filter_vertical_batch::<16>(
                        hev_threshold,
                        interior_limit,
                        mbedge_limit,
                        &mut self.frame.ybuf,
                        (mby * 16) * luma_w + mbx * 16,
                        luma_w,
                    );
                    loop_filter::macroblock_filter_vertical_batch::<8>(
                        hev_threshold,
                        interior_limit,
                        mbedge_limit,
                        &mut self.frame.ubuf,
                        (mby * 8) * chroma_w + mbx * 8,
                        chroma_w,
                    );
                    loop_filter::macroblock_filter_vertical_batch::<8>(
                        hev_threshold,
            ...
            if do_subblock_filtering {
                if self.frame.filter_type {
                    ...
                    for y in (4usize..16 - 3).step_by(4) {
                        loop_filter::subblock_filter_vertical_batch::<16>(
                            hev_threshold,
                            interior_limit,
                            sub_bedge_limit,
                            &mut self.frame.ybuf,
                            (mby * 16 + y) * luma_w + mbx * 16,
                            luma_w,
                        );
                    }

                    loop_filter::subblock_filter_vertical_batch::<8>(
                        hev_threshold,
                        interior_limit,
                        sub_bedge_limit,
                        &mut self.frame.ubuf,
                        (mby * 8 + 4) * chroma_w + mbx * 8,
                        chroma_w,
                    );
                    loop_filter::subblock_filter_vertical_batch::<8>(
                        hev_threshold,
    ...
    fn decode_frame_(mut self) -> Result<Frame, DecodingError> {
        ...
        #[cfg(feature = "profile")]
        {
            self.prof[3] += t.elapsed().as_nanos() as u64;
        }
        ...
        for mby in 0..self.mbheight as usize {
            ...
            for mbx in 0..self.mbwidth as usize {
                ...
                #[cfg(feature = "profile")]
                {
                    self.prof[0] += t.elapsed().as_nanos() as u64;
                    t = std::time::Instant::now();
                }
                ...
                #[cfg(feature = "profile")]
                {
                    self.prof[1] += t.elapsed().as_nanos() as u64;
                }
        ...
        #[cfg(feature = "profile")]
        {
            self.prof[2] += t.elapsed().as_nanos() as u64;
            let total: u64 = self.prof.iter().sum();
            eprintln!(
                "vp8 profile: entropy {:.1}ms ({:.1}%)  predict+idct {:.1}ms ({:.1}%)  loopfilter {:.1}ms ({:.1}%)  header {:.1}ms  total {:.1}ms",
                self.prof[0] as f64 / 1e6,
                self.prof[0] as f64 * 100.0 / total as f64,
                self.prof[1] as f64 / 1e6,
                self.prof[1] as f64 * 100.0 / total as f64,
                self.prof[2] as f64 / 1e6,
                self.prof[2] as f64 * 100.0 / total as f64,
                self.prof[3] as f64 / 1e6,
                total as f64 / 1e6,
            );
        }
