...
The **JPEG XL** single-image container for **remade_ffmpeg_rs** — the file *is*
the codec stream, and this crate models that.

- **Demux + mux** — a one-stream, one-packet container wrapping the whole file.
- Pairs with [`rff-codec-jxl`](https://crates.io/crates/rff-codec-jxl), which is decode-only.
- Pure Rust, no C/FFI.
...
fn main() -> Result<(), Error> {
    ...
    rff_format_jxl::register(&mut formats);

    let file = std::fs::File::open("input.jxl").expect("open input");
    let mut demuxer = formats.open_demuxer("jxl", Box::new(file))?;
