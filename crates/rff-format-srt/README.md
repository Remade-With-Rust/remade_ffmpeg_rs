...
The **SubRip (`.srt`)** subtitle container for **remade_ffmpeg_rs** — our own
code, not a binding.

- **Demux + mux** — numbered cues with `HH:MM:SS,mmm --> HH:MM:SS,mmm` timing and plain text.
- Timing parse/format is shared with WebVTT via [`rff-subtitle`](https://crates.io/crates/rff-subtitle).
- Pure Rust, no C/FFI.
...
fn main() -> Result<(), Error> {
    ...
    rff_format_srt::register(&mut formats);

    let file = std::fs::File::open("input.srt").expect("open input");
    let mut demuxer = formats.open_demuxer("srt", Box::new(file))?;
