//! Dependency-free error type for the Vorbis encoder.
//!
//! The [`Error::Again`] / [`Error::Eof`] pair mirrors FFmpeg's
//! `EAGAIN`/`EOF` codec-drain protocol: a pull call
//! ([`crate::VorbisEncoder::next_packet`]) returns `Err(Again)` when more input
//! is needed before output can be produced, and `Err(Eof)` once the stream has
//! been flushed ([`crate::VorbisEncoder::finish`]) and fully drained. Neither is
//! a failure — they drive the push/pull loop.
...
/// All the ways Vorbis encoding can fail (or signal drain-loop state).
#[non_exhaustive]
