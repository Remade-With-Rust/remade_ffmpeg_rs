...
The **WebP** single-image container for **remade_ffmpeg_rs** — the file *is* the
...
fn main() -> Result<(), Error> {
    ...
    rff_format_webp::register(&mut formats);

    let file = std::fs::File::open("input.webp").expect("open input");
    let mut demuxer = formats.open_demuxer("webp", Box::new(file))?;
