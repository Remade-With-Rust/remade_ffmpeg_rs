...
pub fn quantize(
    ...
    for (idx, &p) in scan.iter().enumerate() {
        let pos = p as usize;
        let step = if idx == 0 { dc_step } else { ac_step } as i64;
        let coeff = coeffs[pos] as i64;
        // Round to nearest: (|coeff|·2^dq_shift + step/2) / step.
        let acoef = (coeff.unsigned_abs() << dq_shift) as i64;
        let level = ((acoef + step / 2) / step) as i32;
        if level != 0 {
            let signed = if coeff < 0 { -level } else { level };
            levels[pos] = signed;
            dqcoeff[pos] = ((signed as i64 * step) >> dq_shift) as i32;
            eob = idx + 1;
        }
    }
    eob
        ...
        for &(n, tx) in &sizes {
            ...
            for &qindex in &[0i32, 32, 96, 200] {
                ...
                for _ in 0..40 {
                    ...
                    let eob = quantize(&coeffs, scan, dc, ac, dq_shift, &mut levels, &mut dqcoeff);

                    // Dequant identity: dqcoeff == (level·step) >> dq_shift.
                    for (idx, &p) in scan.iter().enumerate() {
                        let pos = p as usize;
                        let st = if idx == 0 { dc } else { ac };
                        let want = ((levels[pos] as i64 * st as i64) >> dq_shift) as i32;
