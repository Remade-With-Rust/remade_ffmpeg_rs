...
The **VP9** codec adapter for **remade_ffmpeg_rs**, backed by the in-house
pure-Rust [`rusty_vp9`](https://crates.io/crates/rusty_vp9) engine — no C, no
FFI. Registers `vp9` for both decode and encode.

- **Decoder: bit-exact against all 315 official libvpx conformance vectors** — profiles 0–3, 8/10/12-bit, with AVX2 and NEON kernels.
- **Encoder** with RDO partition/mode decisions, rate control (CBR + two-pass), golden/ALT-REF frames with temporal filtering, and compound (bi-directional) prediction — **validated pixel-exact against libvpx and ffmpeg**.
- Measured honestly: at a matched operating point (both encoders constant-quality, same cq ladder, speed preset and lookahead, read at equal PSNR) it needs **~+36% bitrate** and runs **~3× slower** than libvpx on the Derf CIF set. Keyframe-only BD-rate is ~+0.9% — the gap is the **inter** path, not residual coding.
- Use [`rusty_vp9`](https://crates.io/crates/rusty_vp9) directly if you want the codec without the engine.
...
fn main() -> Result<(), Error> {
    ...
    rff_codec_vp9::register(&mut codecs);

    // Now reachable by id or by FFmpeg-style name.
    let _decoder = codecs.find_decoder(CodecId::Vp9)?;
    let codec = codecs.by_id(CodecId::Vp9).expect("just registered");
