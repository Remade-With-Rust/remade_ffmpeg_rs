...
The **PNG** single-image container for **remade_ffmpeg_rs** — for still images
...
- Pairs with [`rff-codec-png`](https://crates.io/crates/rff-codec-png).
- Pure Rust, no C/FFI.
...
fn main() -> Result<(), Error> {
    ...
    rff_format_png::register(&mut formats);

    let file = std::fs::File::open("input.png").expect("open input");
    let mut demuxer = formats.open_demuxer("png", Box::new(file))?;
