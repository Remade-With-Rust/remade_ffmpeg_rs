...
The **JPEG XL** image codec adapter for **remade_ffmpeg_rs**, backed by the
pure-Rust [`jxl-oxide`](https://crates.io/crates/jxl-oxide) crate.

- **Decode only** — no pure-Rust JPEG XL *encoder* exists yet, so `-c:v jxl` cannot encode.
- MIT/Apache-2.0 backing crate; JPEG XL is royalty-free.
...
fn main() -> Result<(), Error> {
    ...
    rff_codec_jxl::register(&mut codecs);

    // Now reachable by id or by FFmpeg-style name.
    let _decoder = codecs.find_decoder(CodecId::Jxl)?;
    let codec = codecs.by_id(CodecId::Jxl).expect("just registered");
