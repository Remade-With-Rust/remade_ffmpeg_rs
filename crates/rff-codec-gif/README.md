...
The **GIF** still-image codec adapter for **remade_ffmpeg_rs**, backed by the
pure-Rust [`gif`](https://crates.io/crates/gif) crate. First frame only.

- **Decode and encode** the first frame — palette-based, LZW.
- Animation is not yet exposed through the codec layer.
- MIT/Apache-2.0 backing crate; GIF's LZW patents expired in 2004.
...
fn main() -> Result<(), Error> {
    ...
    rff_codec_gif::register(&mut codecs);

    // Now reachable by id or by FFmpeg-style name.
    let _decoder = codecs.find_decoder(CodecId::Gif)?;
    let codec = codecs.by_id(CodecId::Gif).expect("just registered");
