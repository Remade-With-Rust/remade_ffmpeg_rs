...
The **FLV (Flash Video)** container for **remade_ffmpeg_rs** — our own code, not
a binding.

- **Demux + mux** — FLV tags, audio/video tag headers, script data.
- Still the on-the-wire container for RTMP ingest, which is why it is here.
- Pure Rust, no C/FFI.
...
fn main() -> Result<(), Error> {
    ...
    rff_format_flv::register(&mut formats);

    let file = std::fs::File::open("input.flv").expect("open input");
    let mut demuxer = formats.open_demuxer("flv", Box::new(file))?;
