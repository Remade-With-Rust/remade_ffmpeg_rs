...
The native **MP3** elementary-stream container for **remade_ffmpeg_rs** — our own
code, not a binding.

- **Demux + mux** — frame sync, ID3 skipping, Xing/Info header handling.
- A bare MPEG audio elementary stream: there is no real container, which is exactly what this crate models.
- Pure Rust, no C/FFI.
...
fn main() -> Result<(), Error> {
    ...
    rff_format_mp3::register(&mut formats);

    let file = std::fs::File::open("input.mp3").expect("open input");
    let mut demuxer = formats.open_demuxer("mp3", Box::new(file))?;
