...
The **AVIF (AV1 Image File Format)** container for **remade_ffmpeg_rs** — our own
code, not a binding.

- **Demux + mux** — the HEIF/ISOBMFF box structure around an AV1 intra frame.
- **Reads foreign AVIFs too**, not just the ones we write.
- Pairs with [`rff-codec-avif`](https://crates.io/crates/rff-codec-avif).
- Pure Rust, no C/FFI.
...
fn main() -> Result<(), Error> {
    ...
    rff_format_avif::register(&mut formats);

    let file = std::fs::File::open("input.avif").expect("open input");
    let mut demuxer = formats.open_demuxer("avif", Box::new(file))?;
