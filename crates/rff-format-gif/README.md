...
The **GIF** single-image container for **remade_ffmpeg_rs** — the file *is* the
codec stream, and this crate models that.

- **Demux + mux** — a one-stream, one-packet container wrapping the whole file.
- Pairs with [`rff-codec-gif`](https://crates.io/crates/rff-codec-gif) (first frame).
- Pure Rust, no C/FFI.
...
fn main() -> Result<(), Error> {
    ...
    rff_format_gif::register(&mut formats);

    let file = std::fs::File::open("input.gif").expect("open input");
    let mut demuxer = formats.open_demuxer("gif", Box::new(file))?;
