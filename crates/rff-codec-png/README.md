...
The **PNG** still-image codec adapter for **remade_ffmpeg_rs**, backed by the
pure-Rust [`png`](https://crates.io/crates/png) crate. Decode and encode,
RGB/RGBA.

- **Decode and encode** 8-bit RGB and RGBA.
- DEFLATE via `miniz_oxide`/`fdeflate` — pure Rust throughout.
- MIT/Apache-2.0 backing crate; PNG is royalty-free.
...
fn main() -> Result<(), Error> {
    ...
    rff_codec_png::register(&mut codecs);

    // Now reachable by id or by FFmpeg-style name.
    let _decoder = codecs.find_decoder(CodecId::Png)?;
    let codec = codecs.by_id(CodecId::Png).expect("just registered");
