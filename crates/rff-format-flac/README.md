...
The native **FLAC** audio container for **remade_ffmpeg_rs** — our own code, not
a binding.

- **Demux + mux** — `fLaC` magic, `STREAMINFO` and the metadata block chain, then the frame stream.
- Pairs with [`rff-codec-flac`](https://crates.io/crates/rff-codec-flac) for lossless round-trips.
- Pure Rust, no C/FFI.
...
fn main() -> Result<(), Error> {
    ...
    rff_format_flac::register(&mut formats);

    let file = std::fs::File::open("input.flac").expect("open input");
    let mut demuxer = formats.open_demuxer("flac", Box::new(file))?;
