...
The **MPEG-TS (MPEG-2 Transport Stream)** container for **remade_ffmpeg_rs** —
our own code, not a binding.

- **Demux + mux** — 188-byte packets, PAT/PMT tables, PES packetisation.
- The segment format behind [`rff-format-hls`](https://crates.io/crates/rff-format-hls).
- Pure Rust, no C/FFI.
...
fn main() -> Result<(), Error> {
    ...
    let file = std::fs::File::open("input.ts").expect("open input");
    let mut demuxer = formats.open_demuxer("ts", Box::new(file))?;
