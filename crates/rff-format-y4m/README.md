...
The **YUV4MPEG2 (`.y4m`)** raw-video container for **remade_ffmpeg_rs** — the
standard codec-testing interchange format. Our own code.

- **Demux + mux** — the `YUV4MPEG2` header (dimensions, frame rate, interlacing, aspect, colour space) and `FRAME` markers.
- Pairs with [`rff-codec-rawvideo`](https://crates.io/crates/rff-codec-rawvideo); this is how clips enter and leave every encoder benchmark.
- Pure Rust, no C/FFI.
...
fn main() -> Result<(), Error> {
    ...
    rff_format_y4m::register(&mut formats);

    let file = std::fs::File::open("input.y4m").expect("open input");
    let mut demuxer = formats.open_demuxer("y4m", Box::new(file))?;
