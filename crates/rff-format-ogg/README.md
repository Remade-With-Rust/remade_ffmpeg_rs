...
The **Ogg** container for **remade_ffmpeg_rs** — our own code, not a binding.

- **Demux + mux** — page framing, segment tables, granule positions.
- Carries **Opus** and **Vorbis** streams.
- Pure Rust, no C/FFI.
...
fn main() -> Result<(), Error> {
    ...
    rff_format_ogg::register(&mut formats);

    let file = std::fs::File::open("input.ogg").expect("open input");
    let mut demuxer = formats.open_demuxer("ogg", Box::new(file))?;
