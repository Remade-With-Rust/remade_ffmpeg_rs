...
## Corpora
- **Notification sounds** (`/c/Windows/Media/Ring*.wav`): real, but tonal/sustained —
  fire <10% short blocks, so they CANNOT evaluate transient (short-block) or bit-
  allocation work (flat quantization is near-optimal on them).
- **Complex music** (`bash tools/quality/fetch_corpus.sh`): strictly non-copyrighted
  (CC0 / Public Domain) Wikimedia clips — piano (Open Goldberg, CC0), guitar (Leyenda,
  PD), vocal (Mozart aria, PD). Piano/guitar fire **~27% short blocks** and are
  spectrally dense — the right corpus for Floor 2 / reservoir RD. **Match the corpus to
  the brick.**
