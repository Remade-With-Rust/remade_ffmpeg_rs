...
| Audio codec | **mp3** (MPEG-1/2 Layer III) | in-house **decoder + encoder** (**`rusty_mp3`**, also usable standalone) — decoder **bit-exact vs FFmpeg**; encoder MPEG-1/2/2.5, CBR + VBR, joint stereo, block switching, bit reservoir. **0.4.0** caches the psychoacoustic model's FFT twiddle factors (rebuilt on every call before, for ten distinct values) and reuses the mid/side scratch across frames: **1.045× faster encode** (pinned CPU, 33/41 pairs, z = 3.90) at **34% fewer allocations per frame**, output byte-identical |
